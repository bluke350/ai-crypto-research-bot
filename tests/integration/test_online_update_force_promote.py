...
def test_online_update_force_promote(tmp_path, monkeypatch):
    ...
    env = os.environ.copy()
    env['PYTHONPATH'] = '.'
    out = subprocess.run(['python', 'tooling/online_update.py', '--model', 'models/opportunity.pkl', '--out', 'models/opportunity-online-updated.pkl', '--min-rows', '1', '--force-promote'], env=env)
